  ...
  const [mounted, setMounted] = useState(false)
    ...
    if (theme === 'system') {
        if (systemTheme === 'dark') {
            ...
            return (
                <div className='group relative flex cursor-pointer backdrop-blur bg-gray-500/30 w-fit gap-x-10 px-4 py-1 items-center justify-center  border-solid border-2 border-gray-500/50 rounded-full z-[10]' onClick={() => setTheme('light')} >
                    <FaComputer/>
                <div className='hidden xl:group-hover:flex absolute bg-gray-200 text-gray-700 py-1 px-4 mt-[0.5rem] rounded top-8 w-max text-sm'>system-theme</div>
                </div>
        ) 
        }
    } 
    ...
    if (resolvedTheme === 'light') {
        return (
                ...
                <IoMoon />
                <div className='invisible opacity-0 xl:group-hover:opacity-100 xl:group-hover:visible  absolute bg-gray-400/30 text-gray-700 py-1 px-4 mt-[0.5rem] rounded top-8 w-max text-sm transition-all duration-[200ms]'>dark-theme</div>
                </div>
        )
    }
