...
async function Certification() {
  ...
  async function getContent() {
    const queryContent = `*[_type == "certification"]{
                              name,
                              issued,
                              expired,
                              certUrl,
    ...
    const queried_content = await client
      ...
      .then((queried_content) => {
        console.log(
          queried_content,
          queried_content[0].certificationImage.asset.url,
  ...
  const ProjectContainer = (data: sanityQueriedContent) => {
    const DataExpired = () => {
      let expiredOn = parseInt(data?.expired_on);
    ...
    return (
      <div className="flex flex-col">
        ...
        <div className="relative h-[150px] w-full bg-gray-900/[0.2] my-2">
          <Image
            src={data.certificationImage?.asset?.url}
            alt="image"
            fill
            objectFit="contain"
          />
        ...
        <div className="flex justify-between">
          ...
          <div className="text-sm">expires: {data?.expired}</div>
        </div>
        <div className="flex bottom-0 left-0 pr-2 gap-2 h-auto w-full justify-between text-lg">
          ...
          <div className="text-[12px] underline xl:no-underline hover:underline decoration-dotted decoration-blue-500">
            <Link href={data?.cert_external_link}>View certificate</Link>
          </div>
        </div>
      </div>
    );
